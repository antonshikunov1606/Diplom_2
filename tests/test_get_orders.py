...
from test_data import URLS, credentials, Tokens, test_ingredients


class TestGetOrders:
    @allure.title("Получение заказа под авторизованным пользователем")
    def test_get_order_with_authorized_returns_order(self, registration_user, delete_user):
        payload = credentials
    ...
    def test_get_order_without_authorized_not_allowed(self, registration_user, delete_user):
